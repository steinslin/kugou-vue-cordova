<template>
  <div class='lv-overflow-auto con'>
    ...
    <ul v-if='historyList && historyList.length>0' class='lv-bottom-border'>
      <v-touch tag='li' :key='index' class='lv-pd-left-large' v-ripple-btn  v-for='(item,index) in historyList' @tap='searchSong($event,item)' >
...
export default{
  ...
  computed: {
    ...mapState(['historyList', 'hotList', 'skin']),
    ...mapGetters([])
  },
  methods: {
